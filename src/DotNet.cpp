...
public:
    load_assembly_and_get_function_pointer_fn GetLoadAssemblyFunction(std::string const& assembly_entry_point_class, std::string const& assembly_path, std::string config_path, hostfxr_delegate_type load_type) {
    ...
    std::function<int(void*, int)> LoadAssemblyAndEntryPoint(std::string const& assembly_entry_point_class, std::string const& assembly_entry_point_function, std::string const& assembly_path, std::string assembly_runtimeconfig)
    {
        auto load_assembly_function = GetLoadAssemblyFunction(assembly_entry_point_class, assembly_path, assembly_runtimeconfig, hostfxr_delegate_type::hdt_load_assembly_and_get_function_pointer);
        if (load_assembly_function == nullptr)
            return false;
        ...
        return std::function<int(void*, int)>(entryPoint);
    }
    ...
    std::function<int(void*, int32_t)> GetFunctionDelegate(std::string const& full_qualified_class_name, std::string const& static_public_function_name)
    {
        ...
        auto load_assembly_function = GetLoadAssemblyFunction(full_qualified_class_name, it->second.AssemblyPath, it->second.AssemblyConfig, hostfxr_delegate_type::hdt_load_assembly_and_get_function_pointer);
        if (load_assembly_function == nullptr)
            return false;
...
std::function<int(void*, int)> DotNetCoreHost::LoadAssemblyAndEntryPoint(std::string const& assembly_entry_point_class, std::string const& assembly_entry_point_function, std::string const& assembly_path, std::string const& assembly_runtimeconfig)
{
...
std::function<int(void*, int32_t)> DotNetCoreHost::GetFunctionDelegate(std::string const& full_qualified_class_name, std::string const& static_public_function_name)
{
    return _Impl->GetFunctionDelegate(full_qualified_class_name, static_public_function_name);
}
